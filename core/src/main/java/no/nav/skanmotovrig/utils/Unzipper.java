...
public class Unzipper {
    ...
    public static List<FilepairWithMetadata> unzipXmlPdf(File zip) throws IOException {
        return unzipXmlPdf(new ZipInputStream(new FileInputStream(zip)));
    }

    public static List<FilepairWithMetadata> unzipXmlPdf(ZipInputStream zipInputStream) throws  IOException {
    ...
    private static List<FilepairWithMetadata> pairFiles(List<Skanningmetadata> skanningmetadataList, Map<String, byte[]> pdfs, Map<String, byte[]> xmls) {
        return skanningmetadataList.stream().map(metadata -> {
            ...
            if (!pdfs.containsKey(pdfFilnavn)) {
                throw new SkanmotovrigUnzipperFunctionalException("skanmotovrig fant ikke tilhørende pdf-fil til journalpost " + metadata.getJournalpost().getJournalpostId());
            } if (!xmls.containsKey(xmlFilnavn)) {
                throw new SkanmotovrigUnzipperFunctionalException("skanmotovrig fant ikke tilhørende xml-fil til journalpost " + metadata.getJournalpost().getJournalpostId());
            }
